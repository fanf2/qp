...
\abstract{
  ...
  PATRICIA tries called crit-bit trees \cite{djb} have two words of
  overhead per item stored.

  A hash array mapped trie (HAMT) \cite{bagwell} has wide fan-out,
  indexing tree nodes using multiple key bits; each node is compressed
  using the population count of a bitmap to omit NULL child pointers.
  Bagwell sketches an un-hashed pure trie variant of HAMT in section 5
  but doesn't eliminate redundant single-child nodes like crit-bit
  trees.

  QP tries \cite{qp} are similar to crit-bit trees but test 5 bits per
  indirection instead of 1, using the HAMT bitmap POPCOUNT trick to keep
  overhead to at most two 64 bit words per item. QP tries prefetch the
  child pointer array while calculating which child is next; this reduces
