...
my %col;
for my $i (0 .. $#prog) {
	$col{$prog[$i]} = sprintf "\e[3%dm", 1 + $i %7;
}
...
for (my $N = 1 ;; ++$N) {
	...
	printf "%-*s ", $wp + $wf, "round $N";
	printf " | %-31s", $_ for sort keys %stats;
	print "\n";
	for my $file (@file) {
		for my $prog (@prog) {
			printf "%s%-*s %-*s", $col{$prog}, $wp, $prog, $wf, $file;
			for my $test (sort keys %stats) {
				my $mean = $stats{$test}{$prog}{$file}{tot} / $N;
				my $var = $stats{$test}{$prog}{$file}{tot2} / $N - $mean * $mean;
				printf " | \e[2m(%.3f)\e[22m %.3f < %.3f +/- %.3f",
				    $stats{$test}{$prog}{$file}{this},
				    $stats{$test}{$prog}{$file}{min},
				    $mean, $var ** 0.5;
